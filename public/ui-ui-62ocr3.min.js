...
Version: 1
Updated: 2023-12-06T11:03:00.000Z
Author: Peter Širka
...
Component: j-FlowEditor
Version: 1
Updated: 2024-02-06T14:25:00.000Z
Author: Peter Širka
...
Component: j-UIEditor
Version: 1
Updated: 2024-01-24T10:59:00.000Z
Author: Peter Širka
...
COMPONENT('floweditor','left:1;top:1;margin:0;right:0;bottom:0;zindex:30;language:eu;url:https://flow.totaljs.com;closebutton:1',function(self,config,cls){var self=this,iframe,meta={};self.singleton();self.readonly();self.make=function(){var div=document.createElement('DIV');document.body.appendChild(div);self.replace(div);self.aclass(cls+' hidden');self.css({position:'absolute','z-index':config.zindex,left:config.left,top:config.top+config.margin,right:0,bottom:0});self.on('resize + resize2',self.resize);$(W).on('message',function(e){if(!iframe)return;e=e.originalEvent;if(e.source!==iframe.contentWindow)return;var data=e.data;if(typeof(data)==='string')data=PARSE(data);switch(data.TYPE){case'ready':iframe.contentWindow.postMessage(STRINGIFY({TYPE:'load',data:meta.data}),'*');break;case'keypress':case'F1':case'F10':SETTER('shortcuts/exec',data.value||data.TYPE);break;case'save':meta.callback(typeof(data.data)==='object'?JSON.stringify(data.data,null,'\t'):data.data);break;case'close':self.hide();break}})};self.hide=function(){if(iframe){self.find('iframe').remove();iframe=null;self.aclass('hidden');config.close&&self.EXEC(config.close)}};self.setter=function(value){if(!value){self.hide();return}iframe&&self.find('iframe').remove();self.append('<iframe src="{0}&language={1}&darkmode={2}&hideclose={3}&color={4}" scrolling="no" frameborder="0"></iframe>'.format(config.url+'/?socket='+encodeURIComponent(value),config.language,$('body').hclass('ui-dark')?1:0,config.closebutton?'0':'1',encodeURIComponent(DEF.color)));iframe=self.find('iframe')[0];self.resize();self.rclass('hidden')};self.resize=function(){if(iframe){var css={};css.width=WW-config.left-config.right;css.height=WH-config.top-config.bottom;self.css(css);css.width-=2;css.height-=3;$(iframe).css(css)}}});
...
COMPONENT('uieditor','url:https://uibuilder.totaljs.com;margin:0;zindex:30;left:0;top:0;right:0;bottom:0;language:eu;closebutton:1',function(self,config,cls){var self=this,iframe,meta;self.singleton();self.readonly();self.make=function(){var div=document.createElement('DIV');document.body.appendChild(div);self.replace(div);self.aclass(cls+' hidden');self.css({position:'absolute','z-index':config.zindex,left:config.left,top:config.top+config.margin,right:0,bottom:0});self.on('resize + resize2',self.resize);$(W).on('message',function(e){e=e.originalEvent;var data=e.data;if(typeof(data)==='string')data=PARSE(data);if(!data.uibuilder)return;switch(data.TYPE){case'close':setTimeout(self.hide,200);meta.close&&meta.close();config.onclose&&self.EXEC(config.onclose,data.data);break;case'ready':var msg={TYPE:'init',data:meta.data,upload:config.upload?(/^http(s):/i).test(config.upload)?config.upload:true:null,groups:meta.groups,apps:meta.apps,paths:meta.paths,views:meta.views,uibuilder:1};iframe.contentWindow.postMessage(STRINGIFY(msg),'*');break;case'save':meta.save&&meta.save(data.data);config.onsave&&self.EXEC(config.onsave,data.data);break;case'publish':meta.publish&&meta.publish(data.data);config.onpublish&&self.EXEC(config.onpublish,data.data);break;case'render':meta.render&&meta.render(data.data);config.onrender&&self.EXEC(config.onrender,data.data);break;case'upload':self.EXEC(config.upload,data.data,function(response){var msg={TYPE:'callback',data:response,id:data.callbackid,uibuilder:1};iframe.contentWindow.postMessage(STRINGIFY(msg),'*')});break}})};self.hide=function(){if(iframe){self.find('iframe').remove();iframe=null;meta=null;self.aclass('hidden')}};self.iframe=function(){iframe&&self.find('iframe').remove();var url=config.url;url+=url.indexOf('?')===-1?'?':'&';self.append('<iframe src="{0}language={1}&darkmode={2}&hideclose={3}&color={4}" scrolling="no" frameborder="0"></iframe>'.format(url,config.language,$('body').hclass('ui-dark')?1:0,config.closebutton?'0':'1',encodeURIComponent(DEF.color)));iframe=self.find('iframe')[0];self.resize();self.rclass('hidden')};self.setter=function(value){if(!value){self.hide();return}meta=value;self.iframe();self.rclass('hidden')};self.resize=function(){if(!iframe)return;var css={};css.left=config.left;css.top=config.top;css.width=WW-config.right-config.left;css.height=WH-config.top-config.bottom;self.css(css);css.width-=2;css.height-=3;$(iframe).css(css)}});
